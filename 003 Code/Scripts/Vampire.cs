...
public class Vampire : MonoBehaviour, IDamageable
{
    ...
    private bool canAttack, isDead, canWalk, isJumpBackward, isJumpForward, isJump;
    ...
    void Update()
    {
        ...
        if (isJumpForward && distance > attackDistance)
        {
            JumpForward();
            canWalk = false;
            return;
        ...
        {
            canAttack = false;
            int new_num = Random.Range(0, 2);
            DefineJumpBehavior(new_num);
            return;
        }
    ...
    private void DefineBehavior(int num)
    {
        ...
        {
            animator.SetBool("isJumpBackward", true);
            animator.SetTrigger("JumpBack");
        }
    }

    private void DefineJumpBehavior(int num)
    {
        if (num == 0)
        {
    ...
    public void StartJumpBackward()
    {
        isJumpBackward = true;
    }

    public void EndJumpBackward()
    {
        isJumpBackward = false;
        isJump = true;
        canAttack = true;
    ...
    public void StartJumpForward()
    {
        isJumpForward = true;
        Debug.Log("is Jump forward is " + isJumpForward);
    }

    public void EndJumpForward()
    {
        isJumpForward = false;
        isJump = false;
        canAttack = true;
        animator.SetBool("isJumpBackward", false);
        canWalk = true;
        Debug.Log("is Jump forward is " + isJumpForward);
    }
